...
function parsePrivatePaymentChain(conn, arrPrivateElements, callbacks){
	...
	async.forEachOfSeries(
		...
		function(objPrivateElement, i, cb){
			...
			if (i+1 < arrPrivateElements.length){
	...
	parsePrivatePaymentChain(conn, arrPrivateElements, {
		...
		ifOk: function(bAllStable){
			...
			for (var i=0; i<arrPrivateElements.length; i++){
				...
				var input = payload.inputs[0];
				var is_unique = objPrivateElement.bStable ? 1 : null;
				if (!input.type)
				...
				var is_serial = objPrivateElement.bStable ? 1 : null;
...
function updateIndivisibleOutputsThatWereReceivedUnstable(conn, onDone){
	...
	conn.query(
		...
		WHERE outputs.is_serial IS NULL AND units.is_stable=1 AND is_spent=0",
		function(rows){
			if (rows.length === 0)
				return onDone();
			async.eachSeries(
...
function composeIndivisibleAssetPaymentJoint(params){
	...
	var arrBaseOutputs = [{address: params.fee_paying_addresses[0], amount: 0}];
	if (params.base_outputs)
		arrBaseOutputs = arrBaseOutputs.concat(params.base_outputs);
	composer.composeJoint({
		paying_addresses: _.union(params.paying_addresses, params.fee_paying_addresses),
		...
		ifOk: function(objJoint, assocPrivatePayloads, composer_unlock){
			...
			validation.validate(objJoint, {
				...
				ifOk: function(objValidationState, validation_unlock){
					...
					var arrRecipientChains = bPrivate ? [] : null;
					var arrCosignerChains = bPrivate ? [] : null;
							...
							db.query(
								...
								function(outputs){
									...
									async.forEachOfSeries(
										payload.outputs,
										function(output, output_index, cb3){
											if (!output.address || !output.blinding)
												return cb3();
											buildPrivateElementsChain(
